...
ledger_payload="$(node -e 'process.stdout.write(JSON.stringify({request_id:"package-smoke-001",deployment_id:"deploy-package-smoke",project:"package-smoke",version:process.argv[1],environment:"test",status:"verified",operations:{count:0},warnings:{count:0},convergence:{ok:true,operations:0}}))' "$package_version")"
ledger_post() {
  ...
  code="$(node -e 'try{const b=JSON.parse(process.argv[1]);process.stdout.write(b&&b.ok===true?"200":"500")}catch(e){process.stdout.write("500")}' "$out")"
  if [ "$code" != "200" ]; then
