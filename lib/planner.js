...
function planPlugins(desired = [], actual = [], intents = []) {
  const diff = diffNamedCollections(desired.map(comparablePlugin), actual.map(comparablePlugin));
  const operations = [];
  const warnings = [];

  for (const plugin of diff.created) {
    if (plugin.source === "builtin" || plugin.source === "bundled") continue;
  ...
  for (const plugin of diff.updated) {
    if (plugin.desired.source === "builtin" || plugin.desired.source === "bundled") continue;
