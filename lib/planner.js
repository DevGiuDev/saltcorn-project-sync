...
function planTables({ desiredTables = [], actualTables = [], intents = [] }) {
  ...
  const tableRenameIntents = intents.filter(
    (intent) =>
      intent.type === "rename_table" &&
      diff.tables.orphaned.some((table) => table.name === intent.from) &&
      diff.tables.created.some((table) => table.name === intent.to)
  );
  const tableRenameTargets = new Set(tableRenameIntents.map((intent) => intent.to));

  for (const table of diff.tables.created) {
    if (!tableRenameTargets.has(table.name)) operations.push({ action: "create_table", table: table.name, safe: true });
  }
  ...
  for (const orphan of diff.tables.orphaned) {
    ...
    const renameIntent = tableRenameIntents.find((intent) => intent.from === orphan.name);
    if (dropAllowed) operations.push({ action: "drop_table", table: orphan.name, safe: false });
    else if (renameIntent) operations.push({ action: "rename_table", from: orphan.name, to: renameIntent.to, safe: false });
...
function comparablePlugin(plugin = {}) {
  // NOTE: `source` is intentionally NOT compared. It describes where the
  // plugin code lives *on this machine* (local path vs npm registry), so the
  // same logical plugin legitimately reports source "local" in dev and "npm"
  // in prod. Treating that as drift produces perpetual update_plugin ops that
  // can never converge. Only name + version are portable.
  return {
    name: plugin.name,
...
const NON_DEPLOYABLE_PLUGIN_SOURCES = new Set(["builtin", "bundled"]);
const isPluginDeployable = (plugin) => plugin && !NON_DEPLOYABLE_PLUGIN_SOURCES.has(plugin.source);

function planPlugins(desired = [], actual = [], intents = []) {
  // Filter out non-deployable plugins (builtin/bundled) before diffing, so
  // they never produce install/update ops. The filter uses the raw source
  // field, which comparablePlugin no longer carries. A plugin marked as
  // non-deployable in EITHER side is excluded from both, so a builtin plugin
  // reported as npm on the other side does not surface as orphaned drift.
  const nonDeployableNames = new Set();
  for (const p of [...desired, ...actual]) {
    if (!isPluginDeployable(p) && p && p.name) nonDeployableNames.add(p.name);
  }
  const isVisible = (p) => p && isPluginDeployable(p) && !nonDeployableNames.has(p.name);
  const deployableDesired = desired.filter(isVisible).map(comparablePlugin);
  const deployableActual = actual.filter(isVisible).map(comparablePlugin);
  const diff = diffNamedCollections(deployableDesired, deployableActual);
  const operations = [];
  const warnings = [];

  for (const plugin of diff.created) {
    operations.push({ action: "install_plugin", plugin: plugin.name, version: plugin.version || null, safe: true });
  }
  for (const plugin of diff.updated) {
    operations.push({ action: "update_plugin", plugin: plugin.key, version: plugin.desired.version || null, safe: true });
  }
  for (const orphan of diff.orphaned) {
    const dropAllowed = hasIntent(intents, (intent) => intent.type === "drop_plugin" && intent.plugin === orphan.name);
    if (dropAllowed) operations.push({ action: "drop_plugin", plugin: orphan.name, safe: false });
    else warnings.push({ type: "orphaned_plugin", plugin: orphan.name, message: "Preserved by default" });
  }
...
function planProject({ desired = {}, actual = {}, intents = [], env = "dev", backup = false }) {
  ...
  // Plugins FIRST — they register types (UUID, etc.) and table providers
  // that subsequent table creation depends on.
  const pluginPlan = planPlugins(desired.plugins || [], actual.plugins || [], intents);
  operations.push(...pluginPlan.operations);
  warnings.push(...pluginPlan.warnings);
  ...
  // (plugins already planned above, do not duplicate)
...
module.exports = { ENV_POLICIES, requireBackup, typeName, planTables, planNamedKind, comparablePlugin, planPlugins, planIntentOperations, planProject };
