...
function planPlugins(desired = [], actual = [], intents = []) {
  ...
  for (const plugin of diff.created) operations.push({ action: "install_plugin", plugin: plugin.name, version: plugin.version || null, safe: true });
  for (const plugin of diff.updated) operations.push({ action: "update_plugin", plugin: plugin.key, version: plugin.desired.version || null, safe: true });
  for (const orphan of diff.orphaned) {
