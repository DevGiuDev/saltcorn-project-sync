...
const VERSIONED_SCOPE_VERSION = 1;

function scopeObjectsFromState(state = {}) {
  const kinds = [...require("./normalizer").VERSIONED_KINDS, "plugins"];
  const objects = {};
  for (const kind of kinds) {
    objects[kind] = [...new Set((state[kind] || [])
...
function versionedScopeFromState(state = {}) {
  return { version: VERSIONED_SCOPE_VERSION, objects: scopeObjectsFromState(state) };
}

function readVersionedScope(manifest = {}) {
  const scope = manifest.scope;
  if (!scope) return null;
  if (scope.version !== VERSIONED_SCOPE_VERSION) throw new Error(`unsupported manifest scope version: ${scope.version}`);
  if (!scope.objects || typeof scope.objects !== "object") throw new Error("manifest scope.objects is required");
  const objects = {};
  for (const kind of [...require("./normalizer").VERSIONED_KINDS, "plugins"]) {
    const names = scope.objects[kind] || [];
    if (!Array.isArray(names) || names.some((name) => typeof name !== "string" || !name.trim())) {
      throw new Error(`manifest scope.${kind} must be an array of object names`);
    }
    objects[kind] = [...new Set(names.map((name) => name.trim()))].sort((a, b) => a.localeCompare(b));
  }
  return { version: VERSIONED_SCOPE_VERSION, objects };
}
...
function validateManifest(manifest) {
  ...
  if ((!manifest.objects || typeof manifest.objects !== "object") && !manifest.scope) {
    errors.push("objects or scope section is required");
  }
  if (manifest.scope) {
    try {
      readVersionedScope(manifest);
    } catch (error) {
...
module.exports = {
  ...
  VERSIONED_SCOPE_VERSION,
  scopeObjectsFromState,
  versionedScopeFromState,
  readVersionedScope,
  defaultManifest,
  loadManifest,
  validateManifest,
