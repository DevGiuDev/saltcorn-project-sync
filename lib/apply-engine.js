...
function renameTable(state, desired, from, to) {
  const table = findByName(state.tables || [], from);
  if (!table) return;
  const target = desiredTable(desired, to) || {};
  state.tables = removeByName(state.tables || [], from);
  state.tables = upsertByName(state.tables || [], { ...table, ...target, fields: table.fields || [], name: to });
}
...
function applyOperation(state, desired, op) {
  ...
  if (op.action === "rename_table") {
    renameTable(state, desired, op.from, op.to);
    return;
  }
  ...
  if (op.action === "install_plugin" || op.action === "update_plugin") {
    const plugin = findByName(desired.plugins || [], op.plugin);
    if (!plugin) throw new Error(`desired plugin not found: ${op.plugin}`);
    state.plugins = upsertByName(state.plugins || [], plugin);
    return;
  }
  if (op.action === "drop_plugin") {
    state.plugins = removeByName(state.plugins || [], op.plugin);
    return;
  }
