...
const UI_MODES = new Set(["full", "workspace", "deployment"]);
...
const PROCESS_FIELDS = {
  ...
  ui_mode: "SALTCORN_PROJECT_SYNC_UI_MODE",
};
...
function validateEnvironmentConfig(config = {}) {
  ...
  if (config.ui_mode && !UI_MODES.has(config.ui_mode)) errors.push(`ui_mode must be one of: ${Array.from(UI_MODES).join(", ")}`);
  if (config.base_url) {
    try {
