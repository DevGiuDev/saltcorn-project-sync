    ...
    steps: [
      {
        ...
        form: async () => ({
          fields: [
            ...
            {
              name: "fullscreen_branch_warning",
              label: "FullScreen Branch Warning",
              type: "Bool",
              required: false,
              default: false,
              sublabel: "When enabled and there is a branch/tenant mismatch, show a full-screen overlay blocking all pages until the user acknowledges.",
            },
          ],
        }),
      },
...
function renderSettingsPage({ cfg = {}, error = null } = {}) {
  ...
  const fullscreenWarning = cfg.fullscreen_branch_warning || false;

  const errorHtml = error
    ? `<div class="alert alert-danger"><i class="fas fa-exclamation-triangle me-1"></i>${escapeHtml(error)}</div>`
    : '';

  // Check if project_root is valid
  let rootStatus = '';
  if (currentRoot) {
    try {
      const fs = require('node:fs');
    ...
    card({
      ...
      body: `
        ...
        <div class="row">
          ...
          <div class="col-lg-6">
            <h6 class="mb-3"><i class="fas fa-exclamation-triangle me-1"></i>Branch Warning Mode</h6>
            <div class="mb-3">
              <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" id="setting-fullscreen-warning" ${fullscreenWarning ? 'checked' : ''} />
                <label class="form-check-label small" for="setting-fullscreen-warning">
                  <strong>FullScreen Branch Warning</strong>
                </label>
              </div>
              <div class="form-text">When a branch/tenant mismatch is detected, show a full-screen overlay on <strong>all pages</strong> until the user acknowledges. This prevents accidental edits on the wrong tenant.</div>
            </div>
          </div>
        </div>
...
(function(){
  document.getElementById('btn-save-settings').addEventListener('click', function(){
    ...
    fetch('/project-sync/api/settings', {
      ...
      body: JSON.stringify({
        ...
        fullscreen_branch_warning: document.getElementById('setting-fullscreen-warning').checked
      })
    }).then(function(r){ return r.json(); })
    .then(function(j){
      btn.disabled = false;
  ...
  document.getElementById('btn-browse-root').addEventListener('click', function(){
    ...
    fetch('/project-sync/api/settings', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({ project_root: path, fullscreen_branch_warning: document.getElementById('setting-fullscreen-warning').checked })
    }).then(function(r){ return r.json(); })
    .then(function(j){
      if (j.ok) {
  ...
  },
  {
    ...
    callback: async (req, res) => {
      ...
      return apiLocal(req, res, async () => {
        const { project_root, fullscreen_branch_warning } = req.body || {};
        try {
          const stateMod = optionalRequire(["@saltcorn/data/db/state", "@saltcorn/data/dist/db/state"]);
          if (!stateMod) return { ok: false, error: "State module not available" };
          // Always write to ROOT state — config is global, not per-tenant
          const rootState = stateMod.getRootState();
          if (!rootState) return { ok: false, error: "Root state not available" };
          const cfg = rootState.plugin_cfgs["saltcorn-project-sync"] || {};
          cfg.project_root = project_root || "";
          cfg.fullscreen_branch_warning = !!fullscreen_branch_warning;
          // Persist via processSend (Saltcorn worker pattern)
