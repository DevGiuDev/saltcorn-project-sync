...
function guardBranchForRequest(req) {
  ...
  try {
    ...
    if (!guard.ok) {
      ...
      return guard.error + `. Navigate to the correct tenant or switch git branch to match.`;
    }
  } catch {
...
function renderGitPage({ projectRoot, status, branches, log, remoteUrl, branchMap, error }) {
  ...
  // Ahead/behind against remote for all branches
  let aheadBehind = {};
  try {
    const gitOps = require("./git-ops");
    aheadBehind = gitOps.gitAheadBehind(dir, allBranches);
  } catch { /* ignore */ }
  ...
  (function(){
    ...
    document.addEventListener('click', function(e){
      ...
      if (mergeBtn) {
        ...
        fetch('/project-sync/api/branches/merge', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({branch:branch})})
          .then(function(r){return r.json();})
          .then(function(j){
  ...
  },
  {
    ...
    callback: async (req, res) =>
      api(req, res, async () => {
        ...
        const gitOps = require("./git-ops");
        try {
          const result = gitOps.gitMerge(branch, projectRoot);
