...
function applyScopeToTenantObjects(tenantObjects = {}, scope = []) {
  ...
  for (const kind of ["tables", "views", "pages", "triggers", "roles", "plugins", "menu", "settings"]) {
    for (const obj of tenantObjects[kind] || []) {
      ...
      // A tenant object that has no persisted scope row is a candidate, not
      // an included object.  Treating it as checked made newly-created
      // objects look deployable before the user had explicitly saved scope.
