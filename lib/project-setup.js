...
const { stampSyncMeta, readSyncMeta, enrichWithGitInfo, readGitInfo, versionedScopeFromState } = require("./manifest");
...
async function getProjectBySlug(slug) {
  await ensureTables();
  const rows = await dbQuery(`SELECT * FROM ${PROJECTS_TABLE} WHERE slug = $1`, [String(slug || "")]);
  const list = rows.rows || rows;
  return list[0] || null;
}
...
async function buildExportPack(projectId) {
  ...
  const savedScope = await getScope(projectId);
  const { scope } = await addNewScopeEntries(projectId, tenantObjects, savedScope);
...
async function writeProjectToDir(projectId, dir) {
  ...
  const manifest = {
    ...
    scope: versionedScopeFromState(pack),
  };
...
function newScopeEntries(tenantObjects = {}, scope = []) {
  const existing = new Set((scope || []).map(scopeKey));
  return autoDetectScope(tenantObjects)
    .filter((entry) => !existing.has(scopeKey(entry)))
    .map((entry) => ({
      ...entry,
      // Preserve the automatic policy (for example dev plugins remain
      // excluded), while identifying entries discovered after project setup.
      auto_detected: entry.auto_detected || "new_object",
    }));
}

async function addNewScopeEntries(projectId, tenantObjects, scope = []) {
  const additions = newScopeEntries(tenantObjects, scope);
  if (!additions.length) return { scope, additions };
  await ensureTables();
  for (const entry of additions) {
    await dbQuery(
      `INSERT INTO ${SCOPE_TABLE} (project_id, object_type, object_name, included, auto_detected)
       VALUES ($1, $2, $3, $4, $5) ON CONFLICT (project_id, object_type, object_name) DO NOTHING`,
      [projectId, entry.object_type, entry.object_name, entry.included !== false, entry.auto_detected]
    );
  }
  return { scope: dedupeScopeEntries([...scope, ...additions]), additions };
}

async function getScopedTenantObjects(projectId) {
  const tenantObjects = await getTenantObjects();
  const scope = await getScope(projectId);
  const reconciled = await addNewScopeEntries(projectId, tenantObjects, scope);
  return applyScopeToTenantObjects(tenantObjects, reconciled.scope);
}
