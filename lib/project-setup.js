...
const { writeCanonicalJson, ensureProjectDirs, readJsonIfExists, safeFileName } = require("./project-io");
...
function dedupeTenantObjects(tenantObjects = {}) {
  for (const kind of ["tables", "views", "pages", "triggers", "roles", "plugins"]) {
...
function tenantObjectsFromState(state = {}) {
  return dedupeTenantObjects({
    ...
    pages: (state.pages || []).map((page) => ({ name: page.name, title: page.title || "" })),
...
async function getTenantObjects() {
  try {
    return tenantObjectsFromState(await nativeTenantState());
  } catch (_err) {
  ...
  if (Page) {
    try {
      for (const p of await (typeof Page.find === "function" ? Page.find() : [])) {
        result.pages.push({ name: p.name, title: p.title || "" });
      }
    } catch (_e) {}
  }
...
// Keys to strip from field definitions (noise from Saltcorn model)
...
async function buildExportPack(projectId) {
  ...
  if (Table) {
    ...
    for (const t of tenantObjects.tables) {
      ...
      if (Array.isArray(full.fields)) obj.fields = full.fields.map(normalizeField);
...
async function writeProjectToDir(projectId, dir) {
  ...
  // Object collections
  const OBJECT_TYPES = ["tables", "views", "pages", "triggers", "roles"];
  for (const kind of OBJECT_TYPES) {
    ...
    for (const obj of objects) {
      writeCanonicalJson(path.join(objDir, safeFileName(obj.name || obj.role) + ".json"), obj);
    }
  }
