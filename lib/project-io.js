 ...
 * routes and the CLI export: only name/version/source are persisted.
 *
 * Modes:
 * - Full adopt: replaces the plugin entry with {name, version, source}.
...
function pullPluginToLock(projectDir, liveObj, properties) {
  ...
  writeCanonicalJson(lockPath, { plugins });
  return { file: lockPath, mode: "full", adopted_keys: ["version", "source"] };
}
