...
function git(args, dir, opts = {}) {
  try {
    ...
    return { ok: false, stdout, stderr, code: err.status, error: stderr || stdout || err.message };
  }
}
...
function gitCommit(dir, message) {
  ...
  const r = git(["commit", "-m", safeMsg], dir);
  if (!r.ok) return r;
...
function gitPush(dir, remote, branch) {
  const r = branch
    ? git(["push", "--set-upstream", remote || "origin", branch], dir, { timeout: 60_000 })
    : git(["push", "--set-upstream", remote || "origin"], dir, { timeout: 60_000 });
  return r;
}
