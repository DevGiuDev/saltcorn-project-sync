...
const { versionedScopeFromState, readVersionedScope } = require("./manifest");
 ...
 */
function stagedJson(dir, file) {
  const result = git(["show", `:${file}`], dir);
  if (!result.ok) return null;
  try { return JSON.parse(result.stdout); } catch (_err) { return null; }
}

function committedJson(dir, file) {
  const result = git(["show", `HEAD:${file}`], dir);
  if (!result.ok) return null;
  try { return JSON.parse(result.stdout); } catch (_err) { return null; }
}

function scopeKindForPath(file) {
  return String(file || "").match(/^objects\/(tables|views|pages|triggers|roles|menu|settings)\/[^/]+\.json$/)?.[1] || null;
}

function objectNameFromJson(object) {
  return object?.name || object?.role || null;
}

/**
 * Record only object files staged for this commit in portable deployment
 * scope. Reading the index prevents an unstaged object from being referenced,
 * while merging with an existing scope avoids adopting older legacy files.
 */
function syncPortableScope(dir) {
  const manifestPath = path.join(dir, "saltcorn.project.json");
  const manifest = readJsonIfExists(manifestPath, null);
  if (!manifest) return { ok: false, error: "saltcorn.project.json is required to sync portable scope" };
  const unstagedManifest = git(["diff", "--name-only", "--", "saltcorn.project.json"], dir);
  if (unstagedManifest.ok && unstagedManifest.stdout) {
    return { ok: false, error: "Stage or revert saltcorn.project.json changes before recording portable scope" };
  }
  let existingScope;
  try { existingScope = readVersionedScope(manifest); } catch (error) { return { ok: false, error: error.message }; }
  const state = {};
  for (const kind of VERSIONED_KINDS) {
    state[kind] = (existingScope?.objects?.[kind] || []).map((name) => ({ name }));
  }
  state.plugins = (existingScope?.objects?.plugins || []).map((name) => ({ name }));
  const changes = git(["diff", "--cached", "--name-status", "-M"], dir);
  if (!changes.ok) return changes;
  let changedScope = false;
  const add = (kind, name) => {
    if (!name) return;
    if (!(state[kind] || []).some((entry) => objectNameFromJson(entry) === name)) state[kind].push({ name });
    changedScope = true;
  };
  const remove = (kind, name) => {
    if (!name) return;
    state[kind] = (state[kind] || []).filter((entry) => objectNameFromJson(entry) !== name);
    changedScope = true;
  };
  for (const line of changes.stdout.split("\n").filter(Boolean)) {
    const [status, ...paths] = line.split("\t");
    const oldFile = paths[0];
    const newFile = status.startsWith("R") ? paths[1] : paths[0];
    const oldKind = scopeKindForPath(oldFile);
    const newKind = scopeKindForPath(newFile);
    if ((status === "D" || status.startsWith("R")) && oldKind) remove(oldKind, objectNameFromJson(committedJson(dir, oldFile)));
    if (status !== "D" && newKind) {
      const object = stagedJson(dir, newFile);
      if (!object) return { ok: false, error: `Cannot read staged JSON: ${newFile}` };
      add(newKind, objectNameFromJson(object));
    }
  }
  if (!changedScope) return { ok: true, scope: existingScope, skipped: true };
  manifest.scope = versionedScopeFromState(state);
  writeCanonicalJson(manifestPath, manifest);
  const stagedManifest = git(["add", "--", "saltcorn.project.json"], dir);
  if (!stagedManifest.ok) return stagedManifest;
  return { ok: true, scope: manifest.scope };
}

function gitCommit(dir, message, options = {}) {
  if (!message || !message.trim()) {
    return { ok: false, error: "Commit message is required" };
  }
  let scope = null;
  if (options.syncScope) {
    scope = syncPortableScope(dir);
  ...
  return { ok: true, hash: hash.ok ? hash.stdout : null, message: r.stdout, scope: scope?.scope || null };
}
...
function gitPull(dir, remote, branch) {
  const args = branch ? ["pull", "--ff-only", remote || "origin", branch] : ["pull", "--ff-only", remote || "origin"];
  return git(args, dir, { timeout: 60_000 });
}

function gitReset(dir, paths) {
  return paths && paths.length
    ? git(["reset", "HEAD", "--", ...paths], dir)
    : git(["reset", "HEAD"], dir);
}
