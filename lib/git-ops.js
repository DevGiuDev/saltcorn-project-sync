...
const { versionedScopeFromState } = require("./manifest");
...
/**
 * Materialize portable deployment scope from the Git index. Reading the
 * index, rather than the working tree, prevents an unstaged object from being
 * referenced by the manifest that is about to be committed.
 */
function syncPortableScope(dir) {
  ...
  const indexed = git(["ls-files", "--cached"], dir);
  if (!indexed.ok) return indexed;
  const files = indexed.stdout.split("\n").filter(Boolean);
  const state = {};
  for (const kind of VERSIONED_KINDS) {
    state[kind] = [];
    const prefix = `objects/${kind}/`;
    for (const file of files.filter((item) => item.startsWith(prefix) && item.endsWith(".json"))) {
      const object = stagedJson(dir, file);
      if (!object) return { ok: false, error: `Cannot read staged JSON: ${file}` };
      state[kind].push(object);
    }
  }
  const pluginsFile = files.includes("plugins.lock.json") ? stagedJson(dir, "plugins.lock.json") : null;
  state.plugins = Array.isArray(pluginsFile?.plugins) ? pluginsFile.plugins : [];
