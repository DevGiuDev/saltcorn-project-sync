...
function resolveDeploymentScope(desired, configuredScope) {
  if (!(configuredScope instanceof Set)) {
    return { effective: configuredScope || null, additions: [] };
  }
  const effective = new Set(configuredScope);
  const additions = [];
  for (const entry of desiredScopeEntries(desired)) {
    const key = `${entry.object_type}::${entry.object_name}`;
    if (!effective.has(key)) additions.push(entry);
    effective.add(key);
  }
...
async function prepareDeployment({
  ...
  const deploymentScope = resolveDeploymentScope(rawDesired, scopeSet);
  const desired = filterState(rawDesired, deploymentScope.effective);
  const info = adapter.info ? await adapter.info() : {};
  const compatibility = checkSaltcornCompatibility(manifest, info?.saltcorn_version);
  const actual = filterState(
    normalizeProjectExport(await adapter.exportProject({ referenceTables: desired.reference_data || [] })),
    deploymentScope.effective
  );
