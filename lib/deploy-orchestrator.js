...
function resolveDeploymentScope(desired, actual, configuredScope, manifest = {}) {
  ...
  const desiredByKey = new Map(desiredEntries.map((entry) => [entryKey(entry), entry]));
  const actualKeys = new Set(desiredScopeEntries(actual).map(entryKey));
  ...
  // Legacy manifests have no portable scope. Preserve the target allowlist,
  // but infer newly committed objects that do not exist live so a first deploy
  // can create them without a separate Scope/Live Diff round-trip.
  const effective = new Set(configuredScope);
  const additions = [];
  for (const [key, entry] of desiredByKey) {
    if (!effective.has(key) && !actualKeys.has(key)) {
      effective.add(key);
