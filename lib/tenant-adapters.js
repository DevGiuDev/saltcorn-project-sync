...
const CONFIG_SETTING_EXCLUSIONS = new Set([
  "available_packs",
...
const CONFIG_SECRET_KEY_PATTERN = /(secret|password|token|api[_-]?key|private[_-]?key|signing[_-]?key|firebase.*(?:key|services)|credential|bearer|authorization|auth[_-]?key|access[_-]?key|refresh[_-]?key|session[_-]?key)/i;

function configSettingRows(rows = []) {
  return rows.flatMap((row) => {
    const key = row && row.key;
    if (!key || CONFIG_SETTING_EXCLUSIONS.has(key) || CONFIG_SECRET_KEY_PATTERN.test(key)) return [];
    const raw = row.value;
    const value = raw && typeof raw === "object" && Object.prototype.hasOwnProperty.call(raw, "v") ? raw.v : raw;
...
function restAdapter() {
  const capabilities = capabilityReport({
    adapter: "rest",
    methods: { info: true, exportProject: true, applyPlan: true, applyProject: true, backup: true, restore: true, listDeployments: true, getDeployment: true, recordDeployment: true, liveDiff: true },
    resources: {
  ...
  return {
    ...
    async exportProject(options = {}) {
      const url = new URL(endpoint("EXPORT", "/project-sync/api/export"));
      const refs = options.referenceTables || options.reference_tables || [];
      const names = refs.map((ref) => (typeof ref === "string" ? ref : ref && ref.table)).filter(Boolean);
      if (names.length) url.searchParams.set("reference_tables", names.join(","));
    ...
    async backup(context = {}) {
      return fetchJson(endpoint("BACKUP", "/project-sync/api/backup"), { method: "POST", body: JSON.stringify(context) }) || { ok: true };
    },
    ...
    async listDeployments(filters = {}) {
      const url = new URL(endpoint("DEPLOYMENTS", "/project-sync/api/deployments"));
      for (const key of ["project", "environment", "env", "status", "kind", "limit", "offset"]) {
        if (filters[key] !== undefined && filters[key] !== null && filters[key] !== "") url.searchParams.set(key, String(filters[key]));
      }
      return fetchJson(url.toString());
    },
    async getDeployment(id) {
      return fetchJson(`${endpoint("DEPLOYMENTS", "/project-sync/api/deployments")}/${encodeURIComponent(id)}`);
    },
    async recordDeployment(record) {
      return fetchJson(endpoint("DEPLOYMENTS", "/project-sync/api/deployments"), { method: "POST", body: JSON.stringify(record) });
    },
    async liveDiff() {
      return fetchJson(endpoint("LIVE_DIFF", "/project-sync/api/live-diff"));
    },
    async resetSchema() {
...
function commandAdapter() {
  ...
  const capabilities = capabilityReport({
    adapter: "command",
    methods: { info: true, exportProject: true, applyProject: hasApplyCmd, backup: hasBackupCmd, restore: hasRestoreCmd },
    resources: {
  ...
  return {
    ...
    async backup(context = {}) {
      ...
      return runShellJson(cmd, context) || { ok: true };
    },
    async restore(backup) {
...
/**
 * Install or update a single plugin against the real Saltcorn Plugin model.
 *
 * Saltcorn's Plugin model has NO static Plugin.create / Plugin.update.
 * The portable API surface is:
 *   - Plugin.loadAndSaveNewPlugin(pluginInstance, force?) — npm install +
 *     module load + type/viewtemplate registration + DB row upsert. This is
 *     what makes plugin-provided types (e.g. the "JSON" type from the json
 *     plugin) available before subsequent table/field creation.
 *   - instance.upsert() — raw DB row insert/update, no code load.
 *
 * Strategy:
 *   1. If loadAndSaveNewPlugin is available, use it (force on updates). This
 *      is the only path that actually loads plugin code.
 *   2. Else fall back to constructing/updating a Plugin instance and calling
 *      upsert() — persists the row so at least convergence can be reached,
 *      though plugin-provided types will be missing.
 *   3. Else skip with a clear reason.
 *
 * @param {object} Plugin - Saltcorn Plugin model class
 * @param {object} desiredPlugin - { name, source, location, version } from the lock
 * @param {object|null} existing - already-installed Plugin instance, or null
 * @param {boolean} isUpdate - true for update_plugin, false for install_plugin
 * @returns {Promise<{ok: boolean, mode?: string, reason?: string}>}
 */
async function installOrUpdatePlugin(Plugin, desiredPlugin, existing, isUpdate) {
  if (!Plugin) return { ok: false, reason: methodUnavailableReason("plugin", "install", ["Plugin.loadAndSaveNewPlugin", "Plugin.upsert"]) };

  // Build the Plugin instance we want installed. location defaults to the
  // name for npm plugins (Saltcorn's own installer treats location as the
  // npm package name).
  const instanceProps = {
    name: desiredPlugin.name,
    source: desiredPlugin.source || (existing && existing.source) || "npm",
    location: desiredPlugin.location || desiredPlugin.name,
    version: desiredPlugin.version || null,
  };

  // Preferred path: the real installer. It handles both fresh installs and
  // forced reinstalls (force=true is what Saltcorn's own UI uses for updates).
  if (typeof Plugin.loadAndSaveNewPlugin === "function") {
    const target = existing || new Plugin(instanceProps);
    if (isUpdate && desiredPlugin.version) target.version = desiredPlugin.version;
    await Plugin.loadAndSaveNewPlugin(target, Boolean(isUpdate));
    return { ok: true, mode: "loadAndSaveNewPlugin" };
  }

  // Fallback: persist the DB row only. No code is loaded, so plugin-provided
  // types/viewtemplates won't be registered — downstream table/field creation
  // that depends on them may fail. Reach this only on older Saltcorn builds.
  if (typeof Plugin.prototype?.upsert === "function" || (existing && typeof existing.upsert === "function")) {
    const target = existing || new Plugin(instanceProps);
    if (desiredPlugin.version) target.version = desiredPlugin.version;
    if (desiredPlugin.source) target.source = desiredPlugin.source;
    if (desiredPlugin.location) target.location = desiredPlugin.location;
    await target.upsert();
    return { ok: true, mode: "upsert" };
  }
  if (typeof Plugin.upsert === "function") {
    // Some shims expose upsert statically.
    await Plugin.upsert({ ...instanceProps, ...(existing ? { id: existing.id } : {}) });
    return { ok: true, mode: "Plugin.upsert" };
  }

  return { ok: false, reason: methodUnavailableReason("plugin", "install", ["Plugin.loadAndSaveNewPlugin", "plugin.upsert"]) };
}

function nativeSaltcornAdapter(models = loadSaltcornModels()) {
  const db = models.Db || optionalRequireOrNull(["@saltcorn/data/db", "@saltcorn/data/dist/db"]);
  const hasConfigDb = Boolean(db && typeof db.select === "function" && typeof db.update === "function" && typeof db.insert === "function");
  const capabilities = capabilityReport({
    adapter: "saltcorn-native",
    methods: { info: true, exportProject: true, applyPlan: true, backup: Boolean(process.env.SALTCORN_PROJECT_SYNC_BACKUP_CMD), restore: Boolean(process.env.SALTCORN_PROJECT_SYNC_RESTORE_CMD), runMigration: hasConfigDb },
    resources: {
      ...
      plugins: { export: modelHas(models.Plugin, "find"), apply: modelHas(models.Plugin, "loadAndSaveNewPlugin") || modelHas(models.Plugin, "upsert"), destructive: modelHas(models.Plugin, "delete") },
      settings: { export: hasConfigDb || modelHas(models.Setting, "find"), apply: hasConfigDb || modelHas(models.Setting, "create") || modelHas(models.Setting, "update"), destructive: modelHas(models.Setting, "delete") },
      menu: { export: hasConfigDb, apply: hasConfigDb || modelHas(models.Menu, "create") || modelHas(models.Menu, "update"), destructive: modelHas(models.Menu, "delete") },
      reference_data: { export: true, apply: true },
    },
  ...
  return {
    ...
    async exportProject(options = {}) {
      ...
      try {
        if (db && typeof db.select === "function") {
          const menuRows = await db.select("_sc_config", { key: "menu_items" });
          if (menuRows.length) {
            menuConfigured = true;
            const raw = menuRows[0].value;
            const parsed = typeof raw === "string" ? JSON.parse(raw) : raw;
            if (parsed && Array.isArray(parsed.v)) menuItems = parsed.v;
          }
          configSettings = configSettingRows(await db.select("_sc_config", {}));
        }
      } catch (_e) { /* config export best-effort */ }
      ...
      const referenceData = {};
      const referenceTables = options.referenceTables || options.reference_tables || [];
      for (const ref of referenceTables) {
        const tableName = typeof ref === "string" ? ref : ref && ref.table;
        if (!tableName || Object.prototype.hasOwnProperty.call(referenceData, tableName)) continue;
        const table = await findOneByName(Table, tableName);
        if (table && typeof table.getRows === "function") referenceData[tableName] = await table.getRows({});
      }
      ...
      return {
        ...
        menu: menuConfigured ? [{ name: "menu_items", items: menuItems }] : [],
        settings: configSettings.length ? configSettings : await findAll(Setting),
    ...
    async backup(context = {}) {
      ...
      const cmd = process.env.SALTCORN_PROJECT_SYNC_BACKUP_CMD;
      if (cmd) return runShellJson(cmd, context) || { ok: true };
      return {
        ok: true,
        skipped: true,
    ...
    async applyPlan({ desired, plan }) {
      ...
      for (const op of pluginOps) {
        ...
        try {
          const isUpdate = op.action === "update_plugin";
          const existing = Plugin ? await findOneByName(Plugin, plugin.name) : null;
          const result = await installOrUpdatePlugin(Plugin, plugin, existing, isUpdate);
          if (result.ok) {
            // Refresh state after plugin install — new types (UUID, JSON, etc.)
            // and table providers must be registered before creating tables/fields.
      ...
      for (const op of nonTableOps) {
        ...
        if (op.action === "create_field" || op.action === "update_field" || op.action === "alter_field_type") {
          ...
          if (field.type === "Key" && field.reftable_name && !field.reftable_id) {
            ...
            if (reftable) {
              ...
              skipped.push({ op, reason: `reftable "${field.reftable_name}" not found in tenant; skipping Key field ${op.table}.${op.field}` });
              continue;
            }
          }
          ...
          try {
            ...
            // Field creation can fail when the field type is provided by a plugin
            // that isn't loaded in the current runtime (e.g. "JSON" before the
            // json plugin registers it). Skip rather than abort the whole deploy,
            // mirroring Phase 2's field-creation error handling.
            skipped.push({ op, reason: `Field operation failed for ${op.table}.${op.field}: ${fieldErr.message}` });
            continue;
          }
          applied.push(op);
          continue;
        ...
        if (op.action === "drop_plugin") {
          // install_plugin / update_plugin are handled in Phase 0 above; they
          // never reach this loop. drop_plugin is the only plugin op left.
          const existing = await findOneByName(Plugin, op.plugin);
          if (existing && typeof existing.delete === "function") {
            await existing.delete();
            applied.push(op);
          } else {
            skipped.push({ op, reason: methodUnavailableReason("plugin", "delete", ["plugin.delete"]) });
          }
          continue;
        }
        ...
        if (op.action === "raw_sql") {
          if (!db || typeof db.query !== "function") {
            skipped.push({ op, reason: "raw_sql requires db.query (native adapter); use command adapter" });
            continue;
          }
          try {
            ...
            try { await db.query("ROLLBACK"); } catch (_rollbackErr) { /* best effort */ }
            skipped.push({ op, reason: `raw_sql failed (rolled back): ${sqlErr.message}` });
          }
          continue;
        }
        ...
        // Saltcorn settings are stored as { v: value } rows in _sc_config.
        if (op.action === "create_setting" || op.action === "update_setting") {
          const item = (desired.settings || []).find((obj) => obj.name === op.setting);
          const key = item && (item.key || item.name);
          if (item && key && Object.prototype.hasOwnProperty.call(item, "value") && hasConfigDb) {
            const existing = await db.select("_sc_config", { key });
            const value = JSON.stringify({ v: item.value });
            if (existing.length) await db.update("_sc_config", { value }, { key });
            else await db.insert("_sc_config", { key, value }, { pk_name: "key" });
            applied.push(op);
            continue;
          }
        }
            ...
            try {
              if (hasConfigDb) {
                ...
                if (existing.length) {
                  ...
                  await db.insert("_sc_config", { key: "menu_items", value }, { pk_name: "key" });
                }
                applied.push(op);
                storedInConfig = true;
              }
            } catch (_menuErr) {
              // Fall back to the Menu model below on Saltcorn versions that expose one.
            }
          }
          if (storedInConfig) continue;
        }
    ...
    async installPlugin(plugin) {
      ...
      if (!Plugin) return { ok: true, skipped: true, reason: "Plugin model unavailable" };
      const existing = await findOneByName(Plugin, plugin.name);
      const result = await installOrUpdatePlugin(Plugin, plugin, existing, false);
      return result.ok ? { ok: true, mode: result.mode } : { ok: true, skipped: true, reason: result.reason };
    },
  };
}
