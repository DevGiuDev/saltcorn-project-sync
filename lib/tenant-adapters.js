...
function nativeSaltcornAdapter(models = loadSaltcornModels()) {
  ...
  return {
    ...
    async exportProject() {
      const { Table, View, Page, Trigger, Role, Plugin, Menu, Setting } = models;
      return {
    ...
    async applyPlan({ desired, plan }) {
      ...
      const tableOps = plan.operations.filter((op) => op.action === "create_table");
      const nonTableOps = plan.operations.filter((op) => op.action !== "create_table");
      ...
      for (const op of tableOps) {
        ...
        if (!existing) {
          ...
          await Table.create(table.name, table);
