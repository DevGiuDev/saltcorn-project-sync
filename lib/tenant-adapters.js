...
function nativeSaltcornAdapter(models = loadSaltcornModels()) {
  ...
  const capabilities = capabilityReport({
    ...
    methods: { info: true, exportProject: true, applyPlan: true, backup: Boolean(process.env.SALTCORN_PROJECT_SYNC_BACKUP_CMD), restore: Boolean(process.env.SALTCORN_PROJECT_SYNC_RESTORE_CMD) },
    resources: {
