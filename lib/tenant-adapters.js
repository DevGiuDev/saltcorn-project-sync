...
function restAdapter() {
  return {
    name: "rest",
    async info() {
      return fetchJson(endpoint("INFO", "/api/project-sync/info"));
    },
    async exportProject() {
      return fetchJson(endpoint("EXPORT", "/api/project-sync/export"));
    },
    async applyPlan(payload) {
      return fetchJson(endpoint("APPLY", "/api/project-sync/apply"), { method: "POST", body: JSON.stringify(payload) }) || { ok: true };
    },
    async applyProject(state) {
      return fetchJson(endpoint("APPLY", "/api/project-sync/apply"), { method: "POST", body: JSON.stringify({ state }) }) || { ok: true };
    },
    async backup() {
      return fetchJson(endpoint("BACKUP", "/api/project-sync/backup"), { method: "POST", body: "{}" }) || { ok: true };
    },
    async restore(backup) {
      return fetchJson(endpoint("RESTORE", "/api/project-sync/restore"), { method: "POST", body: JSON.stringify(backup) }) || { ok: true };
    },
  };
}
...
function nativeSaltcornAdapter() {
  ...
  return {
    ...
    async info() {
      let saltcornVersion = null;
      try {
        saltcornVersion = require("@saltcorn/data/package.json").version;
      } catch (_err) {
        try {
          saltcornVersion = require("@saltcorn/data/dist/package.json").version;
        } catch (_err2) {
          saltcornVersion = null;
        }
      }
      return { adapter: "saltcorn-native", saltcorn_version: saltcornVersion };
    },
    async exportProject() {
