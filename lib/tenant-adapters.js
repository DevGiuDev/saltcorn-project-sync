...
function restAdapter() {
  return {
    name: "rest",
    async info() {
      return fetchJson(endpoint("INFO", "/project-sync/api/info"));
    },
    async exportProject() {
    ...
    async applyPlan({ desired, plan }) {
      ...
      for (const op of plan.operations) {
        ...
        const handled = await generic.reduce(async (prev, [singular, Model, kind, updateStyle]) => {
          ...
          const existing = Model.findOne ? await Promise.resolve(Model.findOne({ name: item.name })) : null;
          if (existing && typeof existing.update === "function") {
            ...
            skipped.push({ op, reason: `${singular} create/update method unavailable` });
          }
          return true;
