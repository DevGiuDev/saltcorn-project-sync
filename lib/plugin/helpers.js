...
function validateApplyPayload(payload = {}, options = {}) {
  const env = String(payload.env || "dev").trim();
  ...
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/.test(env))
    errors.push(`invalid environment name: ${env}`);
  ...
  const backupRequired = options.backupRequired === undefined
    ? ["test", "prod"].includes(env)
