...
async function findOperationalConfig(projectId, environment = "dev", db = getDb()) {
  ...
  return row ? { ...row.config_json, environment: row.environment, project_id: row.project_id, updated_at: row.updated_at } : null;
}

async function getOperationalConfig(projectId, environment = "dev", db = getDb()) {
  const env = normalizeEnvironmentName(environment);
  return (await findOperationalConfig(projectId, env, db)) || { environment: env, project_id: Number(projectId) };
}
