...
function bearerAuthorized(req) {
  ...
  if (token) {
    ...
    if (suppliedBuffer.length === expectedBuffer.length && crypto.timingSafeEqual(suppliedBuffer, expectedBuffer)) return true;
  }
  return require("./token-store").verifyStoredToken(supplied);
}
...
async function api(req, res, handler) {
  ...
  try {
    ...
    const status = Number.isInteger(err.statusCode) && err.statusCode >= 400 && err.statusCode <= 599 ? err.statusCode : 500;
    return sendJson(res, { ok: false, error: err.message }, status);
  }
}
