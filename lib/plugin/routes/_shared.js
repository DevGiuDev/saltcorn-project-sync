...
const { readJsonIfExists, pullObjectToDisk, pullPluginToLock, deleteObjectFromDisk, safeFileName, writeCanonicalJson } = require("../../project-io");
const { loadChangeIntents, writeDropIntent } = require("../../change-intents");
const { planProject, ENV_POLICIES } = require("../../planner");
const { loadManifest, readSyncMeta, computeFileSyncStatus, stampSyncMeta, versionedScopeFromState } = require("../../manifest");
const { optionalRequire, optionalRequireOrNull } = require("../../tenant-adapters");
 ...
 */
async function exportLivePack(options = {}) {
  const adapter = await nativeAdapterOrError();
  if (adapter.error) return { error: adapter.error };
  return { pack: normalizePack(await adapter.exportProject(options)) };
}
...
module.exports = {
  ...
  loadProjectState, readJsonIfExists, pullObjectToDisk, pullPluginToLock, deleteObjectFromDisk, safeFileName, writeCanonicalJson,
  loadChangeIntents, writeDropIntent, planProject, ENV_POLICIES,
  loadManifest, readSyncMeta, computeFileSyncStatus, stampSyncMeta, versionedScopeFromState,
  optionalRequire, optionalRequireOrNull,
