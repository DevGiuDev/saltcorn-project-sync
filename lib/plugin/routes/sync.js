...
const {
  ...
  readJsonIfExists, pullObjectToDisk, pullPluginToLock, deleteObjectFromDisk, safeFileName, writeCanonicalJson,
...
const routes = [
  {
    ...
    callback: async (req, res) =>
      api(req, res, async () => {
        ...
        // Plugins are stored in plugins.lock.json (not objects/plugins/),
        // so route them through the lock-file upsert instead of the generic
        // per-file writer, which would write a junk file the loader ignores.
        if (kind === "plugins") {
          const result = pullPluginToLock(ctx.projectRoot, liveObj, properties);
