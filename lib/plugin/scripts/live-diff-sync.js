(function(){
  ...
  var projectId = window.SCPS_LIVE_DIFF_PROJECT_ID !== undefined && window.SCPS_LIVE_DIFF_PROJECT_ID !== null && String(window.SCPS_LIVE_DIFF_PROJECT_ID) !== ''
    ? String(window.SCPS_LIVE_DIFF_PROJECT_ID)
    : '';
  function api(path) {
    return projectId ? path + '?project_id=' + encodeURIComponent(projectId) : path;
  }
  function doPull(kind, name, properties){
    return fetch(api('/project-sync/api/pull'), {
      method: 'POST',
  ...
  document.addEventListener('click', function(e){
    ...
    if(deleteBtn){
      e.preventDefault();
      var dk = deleteBtn.dataset.kind, dn = deleteBtn.dataset.name;
      if(!confirm('¿Eliminar ' + dk + '/' + dn + ' del repo?\nSe borrará el fichero y se marcará fuera de scope.')) return;
      deleteBtn.dataset.origClass = deleteBtn.className;
      deleteBtn.disabled = true;
      fetch(api('/project-sync/api/delete-object'), {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({kind: dk, name: dn})
      }).then(function(r){ return r.json(); }).then(function(res){
        deleteBtn.disabled = false;
        if(res.ok){ flash(deleteBtn, 'Eliminado'); setTimeout(function(){ location.reload(); }, 600); }
        else { flash(deleteBtn, 'Error', 'danger'); console.error(res.error); }
      }).catch(function(err){
        deleteBtn.disabled = false;
        flash(deleteBtn, 'Error', 'danger');
        console.error(err);
      });
    }
    // Add to scope button
    ...
    if(addScopeBtn){
      ...
      fetch(api('/project-sync/api/add-to-scope'), {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
    ...
    if(addAllBtn){
      ...
      rows.forEach(function(btn){
        chain = chain.then(function(){
          return fetch(api('/project-sync/api/add-to-scope'), {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
    ...
    if(stampBtn){
      ...
      fetch(api('/project-sync/api/stamp-sync'), {method:'POST'})
        .then(function(r){ return r.json(); })
        .then(function(res){
    ...
    if(versionSaveBtn){
      ...
      if(!newVer) return;
      fetch(api('/project-sync/api/set-version'), {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({version:newVer})})
        .then(function(r){ return r.json(); })
        .then(function(res){
          if(res.ok){
    ...
    if(pullDriftBtn){
      ...
      fetch(api('/project-sync/api/pull-drift'), {method:'POST'})
        .then(function(r){ return r.json(); })
        .then(function(res){
    ...
    if(pushDriftBtn){
      ...
      fetch(api('/project-sync/api/push-drift'), {method:'POST'})
        .then(function(r){ return r.json(); })
        .then(function(res){
    ...
    if(fullPullBtn){
      ...
      fetch(api('/project-sync/api/full-pull'), {method:'POST'})
        .then(function(r){ return r.json(); })
        .then(function(res){
