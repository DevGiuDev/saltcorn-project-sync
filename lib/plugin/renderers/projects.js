 ...
 */
const {
  escapeHtml, card, statCard, emptyState, badge, button, buttonGroup,
  filterableTable, formField, inputGroup, linkCard, pageShell,
  kindIcon, renderTabBar, renderTabPane, textInput, textareaInput,
} = require("../../ui");
...
function renderProjectList(projects) {
  if (!projects.length) {
    return pageShell("Projects", "/project-sync/projects",
      card({ title: "Projects", icon: "fa-folder-open", headerRight: button({ href: "/project-sync/projects/new", color: "primary", size: "sm", icon: "fa-plus", label: "New project" }),
  ...
  const projectCards = projects.map((p) => linkCard({
    href: `/project-sync/projects/${p.id}`,
    title: p.name,
    subtitleHtml: `<code>${escapeHtml(p.slug)}</code>`,
    icon: "fa-folder",
    color: "primary",
    body: `
  ...
  return pageShell("Projects", "/project-sync/projects",
    card({
      ...
      icon: "fa-folder-open",
      iconColor: "teal",
      headerRight: button({ href: "/project-sync/projects/new", color: "primary", size: "sm", icon: "fa-plus", label: "New project" }),
...
<form id="project-form">
  ${formField({
    label: "Name",
    input: textInput({ id: "inp-name", value: project?.name || "", attrs: { required: true } }),
  })}
  ${formField({
    label: "Slug",
    input: textInput({ id: "inp-slug", value: project?.slug || "", placeholder: "auto-generated from name" }),
    helpText: "Used as directory and git repo name.",
  })}
  ${formField({
    label: "Description",
    input: textareaInput({ id: "inp-description", value: project?.description || "", rows: 2 }),
  })}
  ${formField({
    label: "Minimum Saltcorn version",
    input: textInput({ id: "inp-min-version", value: project?.min_version || "1.6.0" }),
  })}
  <div class="d-flex align-items-center" style="gap:.5rem">
    ${button({ type: "submit", color: "primary", label: isNew ? "Create project" : "Save changes" })}
    ${button({ href: "/project-sync/projects", color: "secondary", label: "Cancel" })}
  </div>
</form>
...
function renderProjectDetail(project, scoped) {
  ...
  const projectNavHtml = projectNav({ projectId: project.id, active: "scope" });
  ...
  return pageShell(`Project: ${project.name}`, "/project-sync/projects",
    card({
      title: project.name,
      subtitleHtml: `${badge(project.slug, "primary")} ${project.description ? ` · ${escapeHtml(project.description)}` : ""}`,
      icon: "fa-folder-open",
      iconColor: "primary",
      headerRight: buttonGroup([
        button({ id: "btn-download", color: "success", label: "ZIP", icon: "fa-download" }),
        project.root_path ? button({ id: "btn-write-disk", color: "secondary", outline: true, label: "Write disk", icon: "fa-hdd" }) : "",
        button({ id: "btn-auto-detect", color: "secondary", outline: true, label: "Re-detect", icon: "fa-sync" }),
        button({ id: "btn-save-scope", color: "primary", label: "Save scope", icon: "fa-check" }),
      ]),
      body: `
  ${projectNavHtml}
  ${summaryStats}
