...
function linkCard(project) {
  return `
    <div class="col-md-6 col-xl-4 mb-3">
      <a href="/project-sync/projects/${encodeURIComponent(project.id)}" class="card card-link h-100 text-decoration-none text-reset">
        <div class="card-body">
...
function renderProjectList(projects) {
  ...
  return globalNav("projects") +
    '<div class="card card-max-full-screen"><div class="card-header d-flex justify-content-between align-items-center flex-wrap" style="gap:.5rem"><div><h5 class="mb-0"><i class="fas fa-folder-open me-2"></i>Projects</h5><div class="small text-muted mt-1">Define which objects from this tenant belong to each project</div></div><a href="/project-sync/projects/new" class="btn btn-primary btn-sm"><i class="fas fa-plus me-1"></i>New project</a></div><div class="card-body"><div class="row">' +
...
function objectTable(kind, items) {
  const rows = items.map((obj) => ({
    toggle: `<input type="checkbox" ${obj.included ? "checked" : ""} data-kind="${kind}" data-name="${escapeHtml(obj.name)}" class="scope-toggle form-check-input" />`,
    ...
    auto: obj.auto_detected ? `<span class="badge bg-${obj.included ? "info" : "secondary"}">${escapeHtml(obj.auto_detected)}</span>` : '<span class="text-muted">—</span>',
  }));
...
function renderProjectDetail(project, scoped) {
  ...
  return globalNav("projects") +
    '<div class="card card-max-full-screen">' +
      ...
      '</div>' +
      '<div class="card-body">' +
        projectNav({ projectId: project.id, active: "scope" }) +
        `<div class="row mb-4"><div class="col-md-3 mb-2"><div class="border rounded p-3"><div class="small text-muted">Included</div><div class="h4 mb-0">${totalIncluded}</div></div></div><div class="col-md-3 mb-2"><div class="border rounded p-3"><div class="small text-muted">Excluded</div><div class="h4 mb-0">${totalItems - totalIncluded}</div></div></div><div class="col-md-3 mb-2"><div class="border rounded p-3"><div class="small text-muted">Version</div><div class="h4 mb-0">${escapeHtml(project.min_version || "1.6.0")}</div></div></div><div class="col-md-3 mb-2"><div class="border rounded p-3"><div class="small text-muted">Root</div><div class="h4 mb-0">${project.root_path ? "Set" : "Not set"}</div></div></div></div>` +
