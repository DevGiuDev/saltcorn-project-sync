...
const { projectNav } = require("./project-nav");
...
function renderProjectSyncMeta(projectRoot, projectId = "") {
  let manifest;
  ...
  const projectQuery = projectId !== undefined && projectId !== null && String(projectId) !== ''
  ...
  const gitHint = (git && git.dirty)
    ? '<div class="alert alert-info py-1 px-2 mt-2 mb-0 small"><i class="fas fa-info-circle me-1"></i>Haz <code>git add -A && git commit</code> para registrar los cambios. <a href="/project-sync/git' + projectQuery + '" class="btn btn-outline-dark btn-sm py-0 px-1 ms-1"><i class="fas fa-code-branch me-1"></i>Git panel</a></div>'
    : (git ? '<div class="mt-2"><a href="/project-sync/git' + projectQuery + '" class="btn btn-outline-dark btn-sm py-0 px-1"><i class="fas fa-code-branch me-1"></i>Git panel</a></div>' : '');
...
function subTabBar(activeTab, projectId = "") {
  const query = projectId !== undefined && projectId !== null && String(projectId) !== ''
    ? `?project_id=${encodeURIComponent(projectId)}`
    : '';
  const tabs = [
    { id: "diff", label: '<i class="fas fa-exchange-alt me-1"></i>Diff', href: "/project-sync/live-diff" + query },
    { id: "plan", label: '<i class="fas fa-tasks me-1"></i>Plan', href: "/project-sync/plan-preview" + query },
    { id: "approvals", label: '<i class="fas fa-shield-alt me-1"></i>Approvals', href: "/project-sync/approvals" + query },
  ];
...
function renderLiveDiff({ diff = null, projectRoot = "", projectId = "", error = null, untracked = [] } = {}) {
  const projectNavHtml = projectNav({ projectId, active: "live-diff" });
  const missingRootError = error === "SALTCORN_PROJECT_SYNC_PROJECT_ROOT is not configured" || error === "Project has no root_path";
  if (!projectRoot) {
    if (error && !missingRootError) {
      return pageShell("Live Diff", "/project-sync/live-diff",
        card({ title: "Live Diff", icon: "fa-exchange-alt", body:
          projectNavHtml + `<div class="alert alert-danger"><i class="fas fa-exclamation-triangle me-1"></i>${escapeHtml(error)}</div>`
        })
      );
    }
    return pageShell("Live Diff", "/project-sync/live-diff",
      card({ title: "Live Diff", icon: "fa-exchange-alt", body:
        projectNavHtml + (projectId
  ...
  if (error) {
    return pageShell("Live Diff", "/project-sync/live-diff",
      card({ title: "Live Diff", icon: "fa-exchange-alt", body: projectNavHtml + `<div class="alert alert-danger"><i class="fas fa-exclamation-triangle me-1"></i>${escapeHtml(error)}</div>` })
    );
  }
  ...
  const metaSection = card({
    ...
    icon: "fa-info-circle",
    classExtra: "mb-4",
    body: renderProjectSyncMeta(projectRoot, projectId),
  });
  ...
  const tablesSection = card({
    ...
    classExtra: "mb-4",
    body: renderDiffList((tableDiff && tableDiff.tables) || {}, "table", "tables", fileStatus),
  });
  ...
  const liveDiffContextScript = projectId !== undefined && projectId !== null && String(projectId) !== ""
    ? `<script>window.SCPS_LIVE_DIFF_PROJECT_ID = ${JSON.stringify(projectId)};<\/script>`
    : "";
  ...
  return pageShell("Live Diff", "/project-sync/live-diff",
    card({
      ...
      iconColor: "warning",
      body: liveDiffContextScript + projectNavHtml + `${metaSection}${summaryCards}` +
        diffTabBar +
...
function renderPlanPreview({ plan = null, projectRoot = "", projectId = "", error = null } = {}) {
  const projectNavHtml = projectNav({ projectId, active: "plan" });
  const missingRootError = error === "SALTCORN_PROJECT_SYNC_PROJECT_ROOT is not configured" || error === "Project has no root_path";
  if (!projectRoot) {
    if (error && !missingRootError) {
      return pageShell("Live Diff", "/project-sync/live-diff",
        card({ title: "Plan", icon: "fa-tasks", body: projectNavHtml +
  ...
  return pageShell("Live Diff", "/project-sync/live-diff",
    card({
      ...
      icon: "fa-tasks",
      iconColor: "success",
      body: projectNavHtml + `${summaryCards}${blockedSection}<h6 class="mb-3">Warnings</h6>${warningsSection}<h6 class="mb-3">Operations</h6>${opsSection}`,
    })
  );
}
