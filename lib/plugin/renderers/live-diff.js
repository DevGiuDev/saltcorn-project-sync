...
function planTarget(row = {}) {
  return row.table || row.plugin || row.view || row.page || row.trigger || row.role ||
    row.setting || row.menu || row.field || row.from || row.to || "";
}
...
function renderSyncButton(objKind, diffKind, name, changes, direction) {
  ...
  const deleteBtn = `<button class="btn btn-outline-danger btn-sm delete-btn" data-kind="${kindEsc}" data-name="${escaped}" title="Eliminar del repo"><i class="fas fa-trash me-1"></i>Eliminar</button>`;
  ...
  if (diffKind === "created") {
    // "In Git, missing live": offer recreating it live (push) plus a delete
    // button to confirm the object is gone and remove it from the repo.
    const syncBtn = isToFiles
      ? '<span class="text-muted">—</span>'
      : `<button class="btn ${btnClass} btn-sm push-btn" data-kind="${kindEsc}" data-name="${escaped}"><i class="fas ${iconName} me-1"></i>${label}</button>`;
    return `<div class="d-flex align-items-center" style="gap:.25rem">${syncBtn}${deleteBtn}</div>`;
  }
  if (diffKind === "updated") {
...
function renderLiveDiff({ diff = null, projectRoot = "", projectId = "", uiMode = "full", error = null, untracked = [], sourceStatus = null } = {}) {
  const nav = globalNav("projects");
  const projectTabs = projectNav({ projectId, active: "live-diff", mode: uiMode });
  ...
  return nav +
    '<div class="card card-max-full-screen">' +
      ...
      '<div class="card-body">' +
        projectTabs +
        (sourceStatus?.behind ? `<div class="alert alert-warning"><strong>Checkout is ${sourceStatus.behind} commit(s) behind origin.</strong> This page compares the live tenant with the local checkout, so results may be stale. Generate a deployment plan or pull the branch to fast-forward it safely.</div>` : "") +
...
function renderPlanPreview({ plan = null, projectRoot = "", projectId = "", uiMode = "full", error = null } = {}) {
  const nav = globalNav("projects");
  const projectTabs = projectNav({ projectId, active: "plan", mode: uiMode });
  const missingRootError = error === "SALTCORN_PROJECT_SYNC_PROJECT_ROOT is not configured" || error === "Project has no root_path";
  if (!projectRoot) {
    const body = error && !missingRootError
      ? `<div class="alert alert-danger"><i class="fas fa-exclamation-triangle me-1"></i>${escapeHtml(error)}</div>`
  ...
  const warningsTable = warnings.length ? renderFilterableTable("plan-warnings-table", [
    ...
    { key: (row) => escapeHtml(planTarget(row)), label: "Target" },
...
module.exports = { renderLiveDiff, renderPlanPreview, renderSyncButton };
