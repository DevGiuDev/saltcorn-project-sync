...
function renderApprovals({ approvals = [], projectRoot = "", projectId = "", error = null } = {}) {
  const projectNavHtml = projectNav({ projectId, active: "approvals" });
    ...
    if (error && !missingRootError) {
      return pageShell("Approvals", "/project-sync/approvals",
        card({ title: "Approval Controls", icon: "fa-shield-alt", body:
          projectNavHtml + alert({ color: "danger", icon: "fa-exclamation-triangle", body: escapeHtml(error) })
        })
      );
    }
    return pageShell("Approvals", "/project-sync/approvals",
      card({ title: "Approval Controls", icon: "fa-shield-alt", body:
        projectNavHtml + alert({
          color: "info",
          body: projectId
  ...
  if (error) {
    return pageShell("Approvals", "/project-sync/approvals",
      card({ title: "Approval Controls", icon: "fa-shield-alt", body: projectNavHtml + alert({ color: "danger", icon: "fa-exclamation-triangle", body: escapeHtml(error) }) })
    );
  }
