...
function renderGitPage({ projectRoot, projectId, status, branches, log, remoteUrl, branchMap, error }) {
  const projectNavHtml = projectNav({ projectId, active: "git" });
    ...
    return pageShell("Git", "/project-sync/git",
      card({ title: "Git", icon: "fa-code-branch", body:
        alert({
          color: "info",
          body: projectId
            ? 'This project has no <code>root_path</code> yet. Set the folder on the project first, then open Git again.'
            : 'Open Git from a project with a configured folder, or configure <code>SALTCORN_PROJECT_SYNC_PROJECT_ROOT</code> to enable Git operations.'
        })
      })
    );
  }
  ...
  const aheadBehindHtml = status && (status.ahead > 0 || status.behind > 0)
    ? '<div class="d-flex align-items-center flex-wrap" style="gap:.3rem">' +
      (status.ahead > 0
        ...
        : '') +
      (status.behind > 0
        ? `${badge(`${status.behind} behind`, "orange", { soft: true, textColor: "orange", icon: "fa-arrow-down" })}` +
        ...
        if (!guard.ok) {
          ...
          tenantWarningHtml = alert({
            ...
            body: `
              ...
              ${button({ href: `${expectedUrl}/project-sync/git${projectQuery}`, color: "warning", size: "sm", target: "_blank", icon: "fa-external-link-alt", label: `Open on ${guard.expected}` })}
            `,
          });
  ...
  const gitContextScript = projectId !== undefined && projectId !== null && String(projectId) !== ""
    ? `<script>window.SCPS_GIT_PROJECT_ID = ${JSON.stringify(projectId)};<\/script>`
    : "";
