...
function renderGitPage({ projectRoot, projectId, status, branches, log, remoteUrl, branchMap, uiMode = "full", error }) {
  ...
  const styles = `<style>
    :root{--git-ink:#15202b;--git-panel:#f7f9fb;--git-line:#d8e0e8;--git-cyan:#0b7285;--git-amber:#d97706}
  ...
  return nav + styles + `<div class="scps-git-shell">
    <header class="scps-git-bar"><div class="min-w-0"><div class="fw-bold"><i class="fas fa-code-branch me-2"></i>${escapeHtml(currentBranch)} ${status?.clean ? '<span class="badge bg-success ms-2">clean</span>' : `<span class="badge bg-warning text-dark ms-2">${changeCount} changes</span>`}</div><div class="scps-repo-path mt-1">${escapeHtml(projectRoot)}${remoteUrl?.ok ? ` · ${escapeHtml(remoteUrl.stdout)}` : ""}</div></div><div class="d-flex flex-wrap" style="gap:.35rem"><button id="btn-git-fetch" class="btn btn-sm"><i class="fas fa-sync me-1"></i>Fetch</button><button id="btn-git-pull" class="btn btn-sm"><i class="fas fa-download me-1"></i>Pull${status?.behind ? ` ${status.behind}` : ""}</button><button id="btn-git-push" class="btn btn-sm"><i class="fas fa-upload me-1"></i>Push${status?.ahead ? ` ${status.ahead}` : ""}</button></div></header>
