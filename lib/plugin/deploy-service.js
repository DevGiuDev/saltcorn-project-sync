...
const { fetchRemote, resolveGitCommit, withGitCommitCheckout, fastForwardWorkingBranch, listCommitScopeEntries } = require("../git-source");
...
function publicPrepared(prepared) {
  return {
    ...
    hook_summary: prepared.hook_summary || { pre_deploy_count: 0, post_deploy_count: 0, pending_migrations: 0 },
  };
}
...
async function previewDeployment({ projectId, environment = "dev", sourceRef, refreshSource = true, backupRequested = false, actorId = null } = {}) {
  ...
  const commitEntries = listCommitScopeEntries(context.project.root_path, source.commit);
  const adapter = deploymentAdapter(context);
  const scopeSet = await scopeSetForProject(projectId);
  // Load seed/migration phase plans from the checkout so the preview shows
  // the pre/post-deploy hooks that will run.
  let hookSummary = { pre_deploy_count: 0, post_deploy_count: 0, pending_migrations: 0 };
  const prepared = await withGitCommitCheckout(context.project.root_path, source.commit, (checkout) => {
    try {
      const { loadSeedFiles, loadMigrationFiles, planSeeds, planMigrations, partitionByPhase } = require("../seeds");
      const seedOps = planSeeds(loadSeedFiles(checkout).filter((s) => s.valid).map((s) => ({ seed: s.seed, file: s.file }))).operations;
      const migOps = planMigrations(loadMigrationFiles(checkout).filter((m) => m.valid).map((m) => ({ migration: m.migration, file: m.file }))).operations;
      const { preDeploy, postDeploy } = partitionByPhase([...seedOps, ...migOps]);
      hookSummary = { pre_deploy_count: preDeploy.length, post_deploy_count: postDeploy.length, pending_migrations: migOps.filter((op) => op.run === "once").length };
    } catch (_err) {
      // Checkout may have no seeds/migrations dir; hooks are empty.
    }
  ...
  prepared.hook_summary = hookSummary;
...
async function runDeploymentRequest(requestId, actorId = null) {
  ...
  try {
    ...
    prepared = await withGitCommitCheckout(context.project.root_path, source.commit, (checkout) => {
      // Load seed/migration phase plans from the exact commit being deployed.
      try {
        const { loadSeedFiles, loadMigrationFiles, planSeeds, planMigrations, partitionByPhase } = require("../seeds");
        const seedOps = planSeeds(loadSeedFiles(checkout).filter((s) => s.valid).map((s) => ({ seed: s.seed, file: s.file }))).operations;
        const migOps = planMigrations(loadMigrationFiles(checkout).filter((m) => m.valid).map((m) => ({ migration: m.migration, file: m.file }))).operations;
        ...
        // No seeds/migrations on this checkout; hooks are empty.
      }
      return prepareDeployment({
        sourceDir: checkout,
        adapter,
    ...
    result = await executeDeployment({
      ...
      seedPhasePlans,
      migrationLedger: { project: context.project.slug || context.project.name || "default", env: request.environment },
      deploymentId: `deploy-${request.request_id}`,
      onStep: async (entry) => {
        steps.push(entry);
