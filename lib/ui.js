...
function renderNav(activeHref = "") {
  const links = NAV_ITEMS.map((item) => {
    if (item.href === "__sep__") {
      return '<span class="nav-link px-1 text-muted" aria-hidden="true">|</span>';
    }
  ...
  return `<nav class="nav nav-tabs card-max-full-screen mb-4" style="gap:0;flex-wrap:wrap;">${links}</nav>`;
}
