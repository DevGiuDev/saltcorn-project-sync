 ...
 *
 * Prefer Saltcorn's built-in markup package when available in the host app.
 * Keep a local fallback so the repo test suite still runs outside Saltcorn.
 */

const { optionalRequireOrNull } = require("./tenant-adapters");

const saltcornMarkup = optionalRequireOrNull([
  "@saltcorn/markup",
  "@saltcorn/markup/dist/index",
]);
const saltcornTags = optionalRequireOrNull([
  "@saltcorn/markup/tags",
  "@saltcorn/markup/dist/tags",
]) || saltcornMarkup?.tags || null;
const saltcornLayoutUtils = optionalRequireOrNull([
...
function escapeHtml(value) {
  ...
  return saltcornTags?.escape ? saltcornTags.escape(text) : fallbackEscapeHtml(text);
}
...
function fallbackTag(name, attrs = {}, ...children) {
...
function tag(name, attrs = {}, ...children) {
  const tagFn = saltcornTags?.[name];
  if (typeof tagFn === "function") return tagFn(attrs, ...children);
  if (typeof saltcornTags?.genericElement === "function") {
    return saltcornTags.genericElement(name, attrs, ...children);
  }
  return fallbackTag(name, attrs, ...children);
}
...
function iconAndLabel(iconName, label = "", cls = "") {
  const safeLabel = label ? escapeHtml(label) : "";
  if (saltcornLayoutUtils?.show_icon_and_label) {
    return saltcornLayoutUtils.show_icon_and_label(
      iconName ? classNames("fas", iconName) : undefined,
      safeLabel,
      cls || undefined
    );
  }
  return `${iconName ? icon(iconName, cls) : ""}${iconName && safeLabel ? "&nbsp;" : ""}${safeLabel}`;
}
...
function renderNav(activeHref = "") {
  const links = NAV_ITEMS.map((item) => {
    ...
    return tag("a", { href: item.href, class: classNames("nav-link", active ? "active" : "") }, `${icon(item.icon, "me-1")}${escapeHtml(item.label)}`);
  }).join("");
...
function button({ label = "", href, id, icon: iconName, color = "secondary", outline = false, size, classExtra = "", type = "button", title, target, attrs = {} }) {
  ...
  const content = iconAndLabel(iconName, label, label ? "me-1" : "");
