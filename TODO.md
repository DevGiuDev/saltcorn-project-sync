...
- [ ] Replace the server panel's local-file-only history with an authoritative target-side deployment ledger.
  - [x] Persist sanitized success and failure records in a plugin-owned Saltcorn database table.
...
- [x] Publish `saltcorn-project-sync` to the npm registry.
  ...
  - [x] Configure npm provenance/trusted publishing for subsequent releases.
    - Trusted publishing wired via `.github/workflows/publish.yml` (GitHub Actions OIDC, no long-lived npm token) and validated end-to-end: `0.4.0` published from the `v0.4.0` tag with signed provenance and transparency-log entry.
